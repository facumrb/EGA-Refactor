...
¿Nodos como Genes o Proteínas? Aclarando la Dualidad del Modelo.
Es una observación fundamental y correcta notar que, aunque describimos la Red de Regulación Génica (GRN) como un grafo con genes como nodos, la simulación dinámica en `evaluator_toy.py` opera con las concentraciones de proteínas. Esta aparente contradicción se resuelve al entender que estamos describiendo el sistema en dos niveles de abstracción complementarios y necesarios:
1. El Mapa Estructural (Lógica Regulatoria): La representación de la GRN como un grafo de genes es un mapa estático que describe la lógica de control del sistema. En este nivel, los nodos son genes porque el gen es la unidad fundamental de herencia y regulación; es el "plano" que se lee o se ignora. Las aristas (flechas) representan qué producto génico (generalmente un factor de transcripción, que es una proteína) regula la expresión de qué otro gen. Este mapa nos dice, por ejemplo, "La proteína producida por el Gen A activa la transcripción del Gen B". Es un modelo conceptual que define las reglas del juego.
2. El Proceso Dinámico (Simulación Funcional): La simulación, por otro lado, modela el comportamiento funcional y temporal del sistema. En la célula, los agentes activos que ejecutan las reglas del mapa son las proteínas. Son sus concentraciones las que cambian con el tiempo y determinan el estado celular. Por lo tanto, para simular la dinámica, las variables de nuestro sistema de ecuaciones diferenciales deben ser las concentraciones de proteínas. La simulación responde a la pregunta: "Dadas las reglas del mapa, ¿cómo evolucionan las concentraciones de las proteínas SP1, RUNX2 y ETS1 a lo largo del tiempo?".
